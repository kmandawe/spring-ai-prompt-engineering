package com.kensbunker.springaipromptengineering;
...
public class ZeroAndFewShotTests extends BaseTestClass {
  ...
  @Autowired OpenAiChatProperties openAiChatProperties;
  ...
  /**
   * Zero shot - send the model a single prompt with no hints or examples. Leverages the model's
   * training to generate a response.
   */
  @Test
  void zeroShotPromptTest() {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      PromptTemplate promptTemplate =
          new PromptTemplate(prompt, Map.of("review", UUID.randomUUID() + "\n" + review));
  ...
  void zeroShotPromptTestWithModelOptions() {
    ...
    OpenAiChatOptions openAiChatOptions =
        new OpenAiChatOptions.Builder(openAiChatProperties.getOptions())
            .temperature(1.1) // default is 0.7
   ...
   *
   * <p>Example from 'Language Models are Few-Shot Learners' paper: https://arxiv.org/abs/2005.14165
   */
  String whatpuPrompt =
      """
